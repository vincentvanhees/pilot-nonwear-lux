classifyNonwear = function(data, # assumed to have columns time and Lux, and represent a continuous regular time series
  ...
  aggDay = function(x) {
    ...
    return(c(mean_val = mean(x, na.rm = TRUE),
             n_peaks = n_peaks,
             n_values = length(x),
             p95 = p95(x),
  ...
  # per 16 hour window
  p95_per_16hours = rollApply(data$Lux, window_size_hours = maxZeroSequenceHours,
  ...
  if (length(detect_A) > 0) {
    ...
    data$nonwearA = rollApply(data$nonwearA, window_size_hours = maxZeroSequenceHours,
                              FUN = max, step_size = step_size, N)
  }
  # B. Time windows with consistently a non-zero Lux but hardly any variation for at 
  # least an hour.
  detect_B = which(data$min_per_hour > pmin(min(data$min_per_hour), 1) &
  ...
  time_jumps = which(diff(data$Datetime) > 3600)
  data$nonwearG = 0
  if (length(time_jumps) > 0) {
    data$nonwearG[sort(unique(c(time_jumps, time_jumps + 1)))] = 1
  }
  # Composite score
